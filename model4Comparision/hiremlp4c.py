...
class Downsample_D1(nn.Module):
    """ Downsample stage
    """
    def __init__(self, in_embed_dim, out_embed_dim, patch_size):
        super().__init__()
        assert patch_size == 2, patch_size
        self.proj = nn.Conv2d(in_embed_dim, out_embed_dim, kernel_size=(1, 1), stride=(1, 1))
        self.norm = nn.BatchNorm2d(out_embed_dim)
        self.act = nn.ReLU()
...
class HireMLPNet_X221(nn.Module):
    def __init__(self, layers, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dims=None, mlp_ratios=4., drop_rate=0., attn_drop_rate=0., drop_path_rate=0.,
                 pixel=[2, 2, 2, 2], step_stride=[2, 2, 2, 2], step_dilation=[1, 1, 1, 1],
                 step_pad_mode='c', pixel_pad_mode='c'):
        ...
        for i in range(len(layers)):
            ...
            if i >= len(layers) - 1:
                ...
                network.append(Downsample_D1(embed_dims[i], embed_dims[i + 1], 2))
            else:
                network.append(Downsample(embed_dims[i], embed_dims[i + 1], 2))
...
# -----------------HireMLP series for comparison on 4 datasets------------------------

@register_model
def hire_mlp_tiny_MIN(pretrained=False, **kwargs):
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    pixel = [4, 3, 3, 2]
    ...
    step_pad_mode = 'c'
    pixel_pad_mode = 'c'
    model = HireMLPNet_X221(
        layers, embed_dims=embed_dims, patch_size=3, mlp_ratios=mlp_ratios, pixel=pixel,
        step_stride=step_stride, step_dilation=step_dilation,
        step_pad_mode=step_pad_mode, pixel_pad_mode=pixel_pad_mode, **kwargs)
    # model.default_cfg = _cfg()
    return model
...
@register_model
def hire_mlp_tiny_C10(pretrained=False, **kwargs):
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    pixel = [4, 3, 3, 2]
    ...
    pixel_pad_mode = 'c'
    model = HireMLPNet(
        layers, embed_dims=embed_dims, patch_size=1, mlp_ratios=mlp_ratios, pixel=pixel,
        step_stride=step_stride, step_dilation=step_dilation,
        step_pad_mode=step_pad_mode, pixel_pad_mode=pixel_pad_mode, **kwargs)
    # model.default_cfg = _cfg()
    return model
...
@register_model
def hire_mlp_tiny_C100(pretrained=False, **kwargs):
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    pixel = [4, 3, 3, 2]
...
@register_model
def hire_mlp_tiny_FM(pretrained=False, **kwargs):
    layers = [2, 2, 4, 2]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    pixel = [4, 3, 3, 2]
...
@register_model
def hire_mlp_small_MIN(pretrained=False, **kwargs):
    layers = [3, 4, 10, 3]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    pixel = [4, 3, 3, 2]
...
@register_model
def hire_mlp_small_C10(pretrained=False, **kwargs):
    layers = [3, 4, 10, 3]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    pixel = [4, 3, 3, 2]
...
@register_model
def hire_mlp_small_C100(pretrained=False, **kwargs):
    layers = [3, 4, 10, 3]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    pixel = [4, 3, 3, 2]
...
@register_model
def hire_mlp_small_FM(pretrained=False, **kwargs):
    layers = [3, 4, 10, 3]
    mlp_ratios = [4, 4, 4, 4]
    embed_dims = [64, 128, 320, 512]
    pixel = [4, 3, 3, 2]
