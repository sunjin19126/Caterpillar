...
class sMLPNet_X221(nn.Module):

    def __init__(self, in_chans=3, dim=80, alpha=3, num_classes=10, patch_size=4, image_size=224, depths=[2,8,14,2], dp_rate=0.,
                 **kwargs):
        super(sMLPNet_FM, self).__init__()
        '''
        (B,H,W,C): (B,(image_size// patch_size)**2,dim)
        '''
        ...
        for i in range(len(self.depths)):
            ...
            if i == 0:
                ...
                self.to_patch_embedding.append(nn.Sequential(nn.Conv2d(dim * ratio // 2, dim * ratio, 1, 1, bias=False)))
            else:
                self.to_patch_embedding.append(nn.Sequential(nn.Conv2d(dim * ratio // 2, dim * ratio, 2, 2, bias=False)))
            ...
            for j in range(self.depths[i]):
                ...
                if i == 3:
                    self.token_mix.append(nn.Sequential(DWConvBlock(dim*ratio), sMLPBlock(self.num_patch // (ratio//2), self.num_patch // (ratio//2), dim * ratio)))
                else:
                    self.token_mix.append(nn.Sequential(DWConvBlock(dim * ratio), sMLPBlock(self.num_patch // ratio, self.num_patch // ratio, dim * ratio)))
...
# -----------------sMLPNet series for comparison on 4 datasets------------------------
@register_model
def smlpnet_tiny_MIN(pretrained=False, **kwargs):
    model = sMLPNet_X221(dim=80, alpha=3, patch_size=3, image_size=84, depths=[2,8,14,2], dp_rate=0.0, **kwargs)
    return model

@register_model
def smlpnet_tiny_C10(pretrained=False, **kwargs):
    model = sMLPNet(dim=80, alpha=3, patch_size=1, image_size=32, depths=[2,8,14,2], dp_rate=0.0, **kwargs)
    return model

@register_model
def smlpnet_tiny_C100(pretrained=False, **kwargs):
    model = sMLPNet(dim=80, alpha=3, patch_size=1, image_size=32, depths=[2,8,14,2], dp_rate=0.0, **kwargs)
    return model

@register_model
def smlpnet_tiny_FM(pretrained=False, **kwargs):
    model = sMLPNet_X221(dim=80, alpha=3, patch_size=1, image_size=28, depths=[2,8,14,2], dp_rate=0.0, **kwargs)
    return model