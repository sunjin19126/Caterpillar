...
class PatchMerging_D1(nn.Module):
    """ Patch Merging Layer.
    """
    ...
    def __init__(
        ...
        self.reduction = nn.Linear(dim, self.out_dim, bias=False)

    def forward(self, x):
        B, H, W, C = x.shape
        _assert(H % 1 == 0, f"x height ({H}) is not even.")
        _assert(W % 1 == 0, f"x width ({W}) is not even.")
        x = x.reshape(B, H // 1, 1, W // 1, 1, C).permute(0, 1, 3, 4, 2, 5).flatten(3)
        x = self.norm(x)
        x = self.reduction(x)
        return x
...
class SwinTransformerStage_D1(nn.Module):
    """ A basic Swin Transformer layer for one stage.
    """
    ...
    def __init__(
        ...
        # patch merging layer
        if downsample:
            self.downsample = PatchMerging_D1(
                dim=dim,
                out_dim=out_dim,
                norm_layer=norm_layer,
            )
...
# pyramid model with four stages, where the downsp_rate for the last stage is '1'.
class SwinTransformer_X221(nn.Module):
    """ Swin Transformer
    ...
    def __init__(
        ...
        for i in range(self.num_layers):
            if i != 3:
                ...
                out_dim = embed_dim[i]
                layers += [SwinTransformerStage_D1(
                    dim=in_dim,
                    out_dim=out_dim,
                    input_resolution=(
                        self.patch_grid[0] // (scale//2),
                        self.patch_grid[1] // (scale//2)
                    ),
                    depth=depths[i],
                    downsample=True,
                    num_heads=num_heads[i],
...
def _create_swin_transformer_x221(variant, pretrained=False, **kwargs):
    default_out_indices = tuple(i for i, _ in enumerate(kwargs.get('depths', (1, 1, 3, 1))))
    out_indices = kwargs.pop('out_indices', default_out_indices)

    model = build_model_with_cfg(
        SwinTransformer_X221, variant, pretrained,
        pretrained_filter_fn=checkpoint_filter_fn,
        feature_cfg=dict(flatten_sequential=True, out_indices=out_indices),
        **kwargs)
...
# -----------------Swin series for comparison on 4 datasets------------------------

@register_model
def swin_tiny_patch3_window7_84_MIN(pretrained=False, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        img_size=84, patch_size=3, window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer_x221('swin_tiny_patch3_window7_84_MIN', pretrained=pretrained, **model_kwargs)

@register_model
def swin_tiny_patch1_window4_32_C10(pretrained=False, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        img_size=32, patch_size=1, window_size=4, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer('swin_tiny_patch1_window4_32_C10', pretrained=pretrained, **model_kwargs)

@register_model
def swin_tiny_patch1_window4_32_C100(pretrained=False, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        img_size=32, patch_size=1, window_size=4, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer('swin_tiny_patch1_window4_32_C100', pretrained=pretrained, **model_kwargs)

@register_model
def swin_tiny_patch1_window7_28_FM(pretrained=False, **kwargs):
    """ Swin-T @ 224x224, trained ImageNet-1k
    """
    model_kwargs = dict(
        img_size=28, patch_size=1, window_size=7, embed_dim=96, depths=(2, 2, 6, 2), num_heads=(3, 6, 12, 24), **kwargs)
    return _create_swin_transformer_x221('swin_tiny_patch1_window7_84_FM', pretrained=pretrained, **model_kwargs)
