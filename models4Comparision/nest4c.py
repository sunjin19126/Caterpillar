...
from timm.layers2 import PatchEmbed, Mlp, DropPath, create_classifier, trunc_normal_, _assert
from timm.layers2 import create_conv2d, create_pool2d, to_ntuple
...
# -----------------NesT series for comparison on 4 datasets------------------------

@register_model
def nest_tiny_MIN(pretrained=False, **kwargs):
    """ Nest-T @ 84x84
    """
    model_kwargs = dict(img_size=84, patch_size=3, num_levels=3,
                        embed_dims=(192, 192, 192), num_heads=(3, 3, 3), depths=(4, 4, 4), **kwargs)
    model = _create_nest('nest_tiny', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def nest_tiny_C10(pretrained=False, **kwargs):
    """ Nest-T @ 32x32
    """
    model_kwargs = dict(img_size=32, patch_size=1, num_levels=4,
                        embed_dims=(192, 192, 192, 192), num_heads=(3, 3, 3, 3), depths=(3, 3, 3, 3), **kwargs)
    model = _create_nest('nest_tiny', pretrained=pretrained, **model_kwargs)
    return model

@register_model
def nest_tiny_C100(pretrained=False, **kwargs):
    """ Nest-T @ 32x32
    """
    model_kwargs = dict(img_size=32, patch_size=1, num_levels=4,
                        embed_dims=(192, 192, 192, 192), num_heads=(3, 3, 3, 3), depths=(3, 3, 3, 3), **kwargs)
...
@register_model
def nest_tiny_FM(pretrained=False, **kwargs):
    """ Nest-T @ 28x28
    """
    model_kwargs = dict(img_size=28, patch_size=1, num_levels=3,
                        embed_dims=(192, 192, 192), num_heads=(3, 3, 3), depths=(4, 4, 4), **kwargs)
    model = _create_nest('nest_tiny', pretrained=pretrained, **model_kwargs)
    return model
