...
class StripMLP_Block_XXX7(nn.Module):
...
class StripMLP_Block_XXX4(nn.Module):
    def __init__(self, channels, H, W):
        super().__init__()
        assert W == H
        self.channels = channels
        ...
        self.fc_h = nn.Conv2d(dim, dim, (3, 4), stride=1, padding="same", groups=dim, bias=False)
        self.fc_w = nn.Conv2d(dim, dim, (4, 3), stride=1, padding="same", groups=dim, bias=False)
...
class TokenMixing_XXX7(nn.Module):
    ...
    def __init__(self, C, H, W):
        super().__init__()
        self.smlp_block = StripMLP_Block_XXX7(C, H, W)
...
class TokenMixing_XXX4(nn.Module):
    r""" Token mixing of Strip MLP
    ...
    def __init__(self, C, H, W):
        super().__init__()
        self.smlp_block = StripMLP_Block_XXX4(C, H, W)
        self.dwsc = DepthWise_Conv(C)
...
class BasicBlock_XXX7(nn.Module):
    def __init__(self, in_channel, H, W, alpha, use_dropout=False, drop_rate=0):
        super().__init__()

        self.token_mixing = TokenMixing_XXX7(in_channel, H, W)
...
class BasicBlock_XXX4(nn.Module):
    def __init__(self, in_channel, H, W, alpha, use_dropout=False, drop_rate=0):
        super().__init__()

        self.token_mixing = TokenMixing_XXX4(in_channel, H, W)
        self.channel_mixing = ChannelMixing(in_channel, alpha, use_dropout, drop_rate)
...
class StripMLPNet_XXX7(nn.Module):
    """
    Args:
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
        ...
        super(StripMLPNet_XXX7, self).__init__()
        ...
        for i in range(layers[0]):
            basic = BasicBlock_XXX7(embed_dim, self.patches_resolution[0], self.patches_resolution[1], alpha, use_dropout=use_dropout, drop_rate=drop_rate)
            self.blocks1.append(basic)

        self.blocks2 = nn.ModuleList()
        for i in range(layers[1]):
            basic = BasicBlock_XXX7(embed_dim*2, int(self.patches_resolution[0]/2), int(self.patches_resolution[1]/2), alpha, use_dropout=use_dropout, drop_rate=drop_rate)
            self.blocks2.append(basic)
        
        self.blocks3 = nn.ModuleList()
        for i in range(layers[2]):
            basic = BasicBlock_XXX7(embed_dim*4, int(self.patches_resolution[0]/4), int(self.patches_resolution[1]/4), alpha, use_dropout=use_dropout, drop_rate=drop_rate)
            self.blocks3.append(basic)

        self.blocks4 = nn.ModuleList()
        for i in range(layers[3]):
            basic = BasicBlock_XXX7(embed_dim*8, int(self.patches_resolution[0]/4), int(self.patches_resolution[1]/4), alpha, use_dropout=use_dropout, drop_rate=drop_rate)
        ...
        self.merging3 = nn.Conv2d(embed_dim*4, embed_dim*8, 1, 1, bias=False)

        self.conv_s1_28 = nn.Conv2d(embed_dim*2, embed_dim*4, (2,2), 2, 0, groups=embed_dim*2, bias=False)
        self.conv_s1_14 = nn.Conv2d(embed_dim*4, embed_dim*8, (1,1), 1, 0, groups=embed_dim*4, bias=False)
        self.conv_s2_14 = nn.Conv2d(embed_dim*4, embed_dim*8, (1,1), 1, 0, groups=embed_dim*4, bias=False)
...
class StripMLPNet_XXX4(nn.Module):
    """
    Args:
        img_size (int | tuple(int)): Input image size. Default 224
        patch_size (int | tuple(int)): Patch size. Default: 4
    ...
    def __init__(self, img_size=224, patch_size=4, in_chans=3, num_classes=1000,
                 embed_dim=80, layers=[2, 8, 14, 2], drop_rate=0.5,
                 norm_layer=nn.BatchNorm2d, alpha=3, use_dropout=False, patch_norm=True, **kwargs):
        super(StripMLPNet_XXX4, self).__init__()
        ...
        self.blocks1 = nn.ModuleList()
        for i in range(layers[0]):
            basic = BasicBlock_XXX4(embed_dim, self.patches_resolution[0], self.patches_resolution[1], alpha,
                               use_dropout=use_dropout, drop_rate=drop_rate)
            self.blocks1.append(basic)

        self.blocks2 = nn.ModuleList()
        for i in range(layers[1]):
            basic = BasicBlock_XXX4(embed_dim * 2, int(self.patches_resolution[0] / 2), int(self.patches_resolution[1] / 2),
                               alpha, use_dropout=use_dropout, drop_rate=drop_rate)
            self.blocks2.append(basic)

        self.blocks3 = nn.ModuleList()
        for i in range(layers[2]):
            basic = BasicBlock_XXX4(embed_dim * 4, int(self.patches_resolution[0] / 4), int(self.patches_resolution[1] / 4),
                               alpha, use_dropout=use_dropout, drop_rate=drop_rate)
            self.blocks3.append(basic)

        self.blocks4 = nn.ModuleList()
        for i in range(layers[3]):
            basic = BasicBlock_XXX4(embed_dim * 8, int(self.patches_resolution[0] / 8), int(self.patches_resolution[1] / 8),
                               alpha, use_dropout=use_dropout, drop_rate=drop_rate)
            self.blocks4.append(basic)
...
# @register_model
# def smlpnet_tiny_MIN(pretrained=False, **kwargs):
#     model = sMLPNet_FM(dim=80, alpha=3, patch_size=3, image_size=84, depths=[2,8,14,2], dp_rate=0.0, **kwargs)
#     return model

@register_model
def stripmlpnet_ltiny_MIN(pretrained=False, **kwargs):
    model = StripMLPNet_XXX7(img_size=84, patch_size=3, in_chans=3, embed_dim=80, num_classes=100, layers=[2, 2, 6, 2])
    return model

@register_model
def stripmlpnet_ltiny_C10(pretrained=False, **kwargs):
    model = StripMLPNet_XXX4(img_size=32, patch_size=1, in_chans=3, embed_dim=80, num_classes=10, layers=[2, 2, 6, 2])
    return model

@register_model
def stripmlpnet_ltiny_C100(pretrained=False, **kwargs):
    model = StripMLPNet_XXX4(img_size=32, patch_size=1, in_chans=3, embed_dim=80, num_classes=100, layers=[2, 2, 6, 2])
    return model

@register_model
def stripmlpnet_ltiny_FM(pretrained=False, **kwargs):
    model = StripMLPNet_XXX7(img_size=28, patch_size=1, in_chans=3, embed_dim=80, num_classes=10, layers=[2, 2, 6, 2])
    return model


@register_model
def stripmlpnet_tiny_MIN(pretrained=False, **kwargs):
    model = StripMLPNet_XXX7(img_size=84, patch_size=3, in_chans=3, embed_dim=80, num_classes=100, layers=[2, 2, 12, 2])
    return model

@register_model
def stripmlpnet_tiny_C10(pretrained=False, **kwargs):
    model = StripMLPNet_XXX4(img_size=32, patch_size=1, in_chans=3, embed_dim=80, num_classes=10, layers=[2, 2, 12, 2])
    return model

@register_model
def stripmlpnet_tiny_C100(pretrained=False, **kwargs):
    model = StripMLPNet_XXX4(img_size=32, patch_size=1, in_chans=3, embed_dim=80, num_classes=100, layers=[2, 2, 12, 2])
    return model

@register_model
def stripmlpnet_tiny_FM(pretrained=False, **kwargs):
    model = StripMLPNet_XXX7(img_size=28, patch_size=1, in_chans=3, embed_dim=80, num_classes=10, layers=[2, 2, 12, 2])
    return model
...
#     smlp = stripmlpnet_ltiny_C10(in_chans=3, embed_dim=80, num_classes=num_classes, layers=[2, 2, 6, 2])  # Base
#
#     out = smlp(data)
#
...
#     ops, params = get_model_complexity_info(smlp, (3, 32, 32), as_strings=True,
